...
<body>
<form style="margin: 100px; margin-left: 30%; margin-right: 30%;" method="post" action="/course">
    <input type="hidden" name="_token" value="{{ csrf_token() }}">
    <div class="form-group">
        <label for="name">Name</label>
        <input type="text" class="form-control" id="name" name="name" placeholder="Amozesh 3tar" value="{{old('name')}}">
    </div>
    @error('name')
    ...
    <div class="form-group" style="margin-top: 5px;">
        ...
        <select style="margin-top: 3px;" class="form-select" aria-label="Default select example" name="maestro" id="maestro">
            @foreach($maestros as $maestro)
                <option value="{{$maestro->id}}" {{(old('maestro') == $maestro)?'selected':''}}>{{$maestro->name}}</option>
            @endforeach
        </select>
    </div>
